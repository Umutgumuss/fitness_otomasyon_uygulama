...
-- antrenman  prosedürü --
CREATE PROCEDURE InsertAntrenman(
    IN p_UyeID INT,
    IN p_AntrenorID INT,
    IN p_AntrenmanTarihi DATE,
    IN p_EgzersizAdi VARCHAR(100),
    IN p_SetSayisi INT,
    IN p_TekrarSayisi INT,
    IN p_Agirlik DECIMAL(5, 2)
)
BEGIN
    INSERT INTO Antrenmanlar (UyeID, AntrenorID, AntrenmanTarihi, EgzersizAdi, SetSayisi, TekrarSayisi, Agirlik)
    VALUES (p_UyeID, p_AntrenorID, p_AntrenmanTarihi, p_EgzersizAdi, p_SetSayisi, p_TekrarSayisi, p_Agirlik);
END //
...
CREATE PROCEDURE InsertKatilim(
    IN p_UyeID INT,
    IN p_KatilimTarihi DATE
)
BEGIN
    INSERT INTO Katilim (UyeID, KatilimTarihi) VALUES (p_UyeID, p_KatilimTarihi);
END //
...
CREATE PROCEDURE GetUyeBilgileri(
    IN p_UyeID INT
)
BEGIN
    SELECT * FROM Uyeler WHERE UyeID = p_UyeID;
END //

DELIMITER ;

-- antrenman başlangıç / bitiş tarihi --
DELIMITER //

CREATE PROCEDURE GetAntrenmanlarByDateRange(
    IN p_UyeID INT,
    IN p_BaslangicTarihi DATE,
    IN p_BitisTarihi DATE
)
BEGIN
    SELECT *
    FROM Antrenmanlar
    WHERE UyeID = p_UyeID
      AND AntrenmanTarihi BETWEEN p_BaslangicTarihi AND p_BitisTarihi;
END //
